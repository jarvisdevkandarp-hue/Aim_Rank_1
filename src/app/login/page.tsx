    ...
    <div className="flex-1 flex flex-col w-full px-8 sm:max-w-md justify-center gap-2 m-auto mt-20">
      <Card className="border-slate-800 bg-slate-900/50 text-white">
        <CardHeader className="text-center">
          <CardTitle className="text-2xl font-bold">Mission Login</CardTitle>
          <CardDescription className="text-slate-400">
            Enter your coordinates to access the Operating System.
          </CardDescription>
        </CardHeader>
          ...
          <form className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                name="email"
                type="email"
                placeholder="topper@mission2027.com"
                required
            ...
            <div className="flex gap-2 pt-2">
              <Button formAction={signInWithEmail} className="flex-1 bg-indigo-600 hover:bg-indigo-500">
                Login
              </Button>
              <Button formAction={signUpWithEmail} variant="outline" className="flex-1 border-slate-700 text-white hover:bg-slate-800">
                Sign Up
              </Button>
            </div>
            {searchParams?.message && (
              <p className="mt-4 p-4 bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-center text-sm rounded-lg">
                {searchParams.message}
              </p>
            )}
          </form>
      ...
      <p className="text-xs text-center text-slate-500 mt-4">
        Mission protocols strictly enforced. Authorized access only.
      </p>
    </div>
  )
}
